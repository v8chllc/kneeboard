...
5. **Publish.** Push the branch and open a **draft** pull request per repository, cross-linked where a pair is in scope, with the body `references/run-state.md` defines: closing its own repository issue only when this pull request completes it, and otherwise naming it on a `Part of #<n>` line, and naming the run issue without a closing keyword. Append a record naming each pull request as soon as it opens, before anything else runs against it.
